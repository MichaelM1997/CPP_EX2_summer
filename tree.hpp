...
public:
    ...
    void add_sub_node(Node<T> &parent, Node<T> &ch)
    {
        ...
        // check if the number of children of the node is < k       
        if (parent.children.size() < k)
        {
            parent.add_child(&ch); // is yes we add the node
        }
...
public:
    ...
    public:
        ...
        // constracto for the preorder iterator
        PreOrderIterator(Node<T> *root)
        {
            // if root exist we add the root to the stack
            if (root)
            {
                st1.push(root);
            }
        ...
        Node<T> *operator->() const
        {
            return st1.top();// return the pointer
        }
    ...
    public:
        ...
        {
            // if root exist we add the root to the stack1
            if (root)
            {
                st1.push(root);
        ...
        Node<T> *operator->() const
        {
            return st2.top();// return the pointer
        }
        ...
        PostOrderIterator &operator++()
        {
            ...
            st2.pop();
            return *this;// return the top of the stack2
        }
    };
    ...
    public:
        ...
        {
            // if root exist we add the root of the queu of bfd
            if (root)
            {
                queu_bfs.push(root);
            }
        ...
        Node<T> *operator->() const
        {
            return queu_bfs.front();// return the pointer
        }
    ...
    private:
        ...
        void push_left(Node<T> *node)
        {
            ...
            while (node)// check if node is not null
            {
    ...
    public:
        ...
        InOrderIterator &operator++()
        {
            ...
            if (!node->children.empty())
            {
                // do push left on each children of the node
    ...
    public:
        ...
        {
            // if root exist add the root to the stack
            if (root)
            {
                st1.push(root);
            }
    ...
    public:
        ...
        Node<T> *operator->() const
        {
            return vec_heap_iterator.front();// return the pointer 
        }
